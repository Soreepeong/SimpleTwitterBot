...
$title="Random Replies of ".$user['screen_name']." - $title";
	...
	if($_POST['act']=="add"){
		...
		if(isset($_POST['one_tweet_per_one_line'])){
			foreach(explode("\n", $_POST['s_tweet']) as $tweet){
				$tweet=trim($tweet);
				if(strlen($tweet)>0)
					$userc->addRandomReply($rep_from_user, $tweet, $start_time, $end_time);
			}
		}else{
		...
		$userc->clearRandomReplies();
	}else if($_POST['act']=="delete"){
		$userc->deleteRandomReply($_POST['id']);
	}
	break;
}
...
function printContent(){
	...
	$user=$_SESSION['users'][$cur_user];
	$id=$user['id'];
	$stmt = $conn->prepare("SELECT count(*) FROM {$db_prefix}_randomreply WHERE n_user=:n_user");
	...
	<div style="font-size:15pt;font-weight:bold;">
		<img style="width:32px;height:32px;vertical-align:middle;" src="<?php echo $user['screen_image']?>" /> <?php echo $user['screen_name']; ?>: Total <?php echo $row[0]?> Random Replies
	</div>
	<fieldset style="width:480px;float:left;">
		<legend>Add</legend>
		<form method="post" action="/user/<?php echo $user['screen_name']."/".$id?>/random-replies">
			<input type="hidden" name="act" value="add" />
			<div style="padding:5px;">
				<textarea id="s_tweet" name="s_tweet" style="width:100%;height:48px;"><?php if(isset($_POST['s_tweet'])) echo htmlspecialchars($_POST['s_tweet']); else{ ?>@{mentioner.id} <?php } ?></textarea>
				...
				<div style="display:block;height:32px;">
					...
					<input type="button" value="Test" style="height:32px;width:80px;" onclick="testInputString('#s_tweet', '#s_tweet_test_result', 'random-replies');return false;" />
	...
	<fieldset style="width:240px;height:100px;float:left;">
		...
		<form method="post" action="/user/<?php echo $user['screen_name']."/".$id?>/random-replies" onsubmit="return confirm('Do you really want to clear this list?');">
	...
	<fieldset style="clear:both">
		...
		<table style="width:100%">
			...
			<tbody>
				<?php
				$stmt = $conn->prepare("SELECT * FROM {$db_prefix}_randomreply WHERE n_user=:id");
