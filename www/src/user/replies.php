...
$title="Replies of ".$user['screen_name']." - $title";
	...
	if($_POST['act']=="add"){
		...
		if(isset($_POST['one_tweet_per_one_line'])){
			foreach(explode("\n", $_POST['s_tweet']) as $tweet){
				$tweet=trim($tweet);
				if(strlen($tweet)>0)
					$userc->addReply($rep_from, $rep_from_user, $tweet, $start_time, $end_time);
			}
		}else{
...
function printContent(){
	...
	$user=$_SESSION['users'][$cur_user];
	$id=$user['id'];
	$stmt = $conn->prepare("SELECT count(*) FROM {$db_prefix}_reply WHERE n_user=:n_user");
	...
	<div style="font-size:15pt;font-weight:bold;">
		<img style="width:32px;height:32px;vertical-align:middle;" src="<?php echo $user['screen_image']?>" /> <?php echo $user['screen_name']; ?>: Total <?php echo $row[0]?> Replies
	</div>
	<fieldset style="width:480px;float:left;">
		<legend>Add</legend>
		<form method="post" action="/user/<?php echo $user['screen_name']."/".$id?>/replies">
			<input type="hidden" name="act" value="add" />
			<div style="padding:5px;">
				<b>Trigger (RegEx (preg), Including Mention): </b><a href="http://kr1.php.net/manual/en/pcre.pattern.php">(Help)</a><br />
				<textarea id="s_trigger" name="s_trigger" style="width:100%;height:48px;"><?php if(isset($_POST['s_trigger'])) echo htmlspecialchars($_POST['s_trigger']); else{ ?>/^@[A-Za-z0-9\-]+(?![A-Za-z0-9_])\s*(.*)$/iu<?php } ?></textarea>
				<b>Answer (Replace to): </b><br />
				<textarea id="s_tweet" name="s_tweet" style="width:100%;height:48px;"><?php if(isset($_POST['s_tweet'])) echo htmlspecialchars($_POST['s_tweet']); else{ ?>@{mentioner.id} $1<?php } ?></textarea>
				...
				<div style="display:block;height:48px;">
					...
					Use as <b>{mentioner.id}</b>, <b>{mentioner.name}</b>
				</div>
				<div style="display:block;height:32px;">
					...
					<input type="text" id="s_test_tweet" style="width:160px;" value="@youraccount Hello!" />
				</div>
			</div>
		</form>
	</fieldset>
	<fieldset style="width:240px;height:100px;float:left;">
		...
		<form method="post" action="/user/<?php echo $user['screen_name']."/".$id?>/replies" onsubmit="return confirm('Do you really want to clear this list?');">
	...
	<fieldset style="clear:both">
		...
		<table style="width:100%">
			...
			<tbody>
				<?php
				$stmt = $conn->prepare("SELECT * FROM {$db_prefix}_reply WHERE n_user=:id");
